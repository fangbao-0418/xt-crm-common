  ...
  render(h) {
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'status',
        render: (text, record) => {
          return <span>{arrToEnum(status)[text]}</span>;
        }
      },
      {
        title: '中奖类型',
        ...
        <Card>
          <Form layout="inline">
            <FormItem label="抽奖码编号">
              {getFieldDecorator('ticketCode')(<Input placeholder="请输入抽奖码编号" />)}
            </FormItem>
            <FormItem label="主订单号">
              {getFieldDecorator('mainOrderNo')(<Input placeholder="请输入主订单号编号" />)}
            </FormItem>
            <FormItem label="下单手机号">
              {getFieldDecorator('buyerPhone')(<Input placeholder="请输入下单手机号" />)}
            </FormItem>
            ...
            <Row>
              <Col span={12}>
                <DisableModal
                  ...
                  selRow={this.state.selectedRows}
                  btntext={'批量失效'}
                ></DisableModal>
                <Button
                  type="primary"
