    ...
    const columns = [
        {
            ...
            render(_, record) {
                return (
                    <div>
                        <a onClick={() => scope.onEdit(record)} style={{ marginRight: 10 }}>编辑</a>
                        <a onClick={() => scope.onChangeStatus(record)}>{record.status === 1 ? '禁用' : '启用'}</a>
                    </div>
                )
            }
        }
    ...
    renderForm = () => {
        ...
        return (
            <Form layout="inline">
                ...
                <FormItem label="状态">
                    {
                        getFieldDecorator('status', {
                            ...
                            <Select style={{ width: 100 }}>
                                {
                                    statusList.map(item => <Option value={item.value} key={item.value}>{item.key}</Option>)
                                }
                            </Select>
                        )
                    }
                ...
                <FormItem>
                    <Button type="primary" onClick={this.onShowModal}>添加成员</Button>
                </FormItem>
            </Form>
        )
    }
