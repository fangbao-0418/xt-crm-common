...
class GoodsEdit extends React.Component {
  ...
  getGoodsDetial = (list) => {
    ...
    getGoodsDetial({ productId: id }).then((res = {}) => {
      ...
      this.setState({
        ...
        videoCoverUrl: res.videoCoverUrl,
  ...
  render() {
    ...
    const storeName = (supplier || []).find(v => v.id === storeId) || '';
    const {
      match: {
        params: { id },
      },
    ...
    return (
      <Form {...formLayout}>
        <Card title="添加/编辑商品">
          <Form.Item label="商品名称">{this.state.productName}</Form.Item>
          ...
          <Form.Item label="商品简称">{productShortName}</Form.Item>
          <Form.Item label="商品编码">{productCode || '无'}</Form.Item>
          <Form.Item label="商品简介">{description}</Form.Item>
          <Form.Item label="商品条码">{barCode || '无'}</Form.Item>
          {/* <Form.Item label="供货商">{storeName}</Form.Item> */}
          <Form.Item label="供应商商品ID">{storeProductId}</Form.Item>
          {
            interceptionVisible ? 
