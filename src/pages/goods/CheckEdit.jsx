...
import { mapTree, treeToarr, formatMoneyBeforeRequest, parseQuery } from '@/util/utils';
import {
  map,
  size,
  concat,
...
class GoodsEdit extends React.Component {
  readOnly = parseQuery().readOnly === '1'
  specs = []
  state = {
    speSelect: [],
    spuName: [],
  ...
  getGoodsDetial = (list) => {
    ...
    getGoodsDetial({ productId: id }).then((res = {}) => {
      ...
      setFieldsValue({
        ...
        productCode: this.setDefaultValue(res.productCode),
        ...
        storeId: res.storeId,
        status: res.status,
        bulk: this.setDefaultValue(res.bulk),
        weight: this.setDefaultValue(res.weight),
        ...
        barCode: this.setDefaultValue(res.barCode),
        bannerUrl: initImgList(res.bannerUrl),
        returnPhone: res.returnPhone,
        listImage,
        productImage,
        storeProductId: this.setDefaultValue(res.storeProductId),
        categoryId,
      ...
      <Form {...formLayout}>
        <Card title="添加/编辑商品">
          <Form.Item label="商品名称">
            ...
            })(<Input placeholder="请输入商品名称" className={this.readOnly ? styles['readOnly']: ''} readOnly={this.readOnly}/>)}
          </Form.Item>
          <Form.Item label="商品类目">
            ...
            })(<Cascader className={this.readOnly ? styles['readOnly']: ''} options={this.state.categoryList} readOnly={this.readOnly} placeholder="请输入商品类目" />)}
          </Form.Item>
          <Form.Item label="商品简称">
            ...
            })(<Input placeholder="请输入商品简称" className={this.readOnly ? styles['readOnly']: ''} readOnly={this.readOnly}/>)}
          </Form.Item>
          <Form.Item label="商品编码">
            {getFieldDecorator('productCode')(<Input placeholder="请输入商品编码" className={this.readOnly ? styles['readOnly']: ''} readOnly={this.readOnly}/>)}
            </Form.Item>
            ...
            })(<Input placeholder="请输入商品简介" className={this.readOnly ? styles['readOnly']: ''} readOnly={this.readOnly}/>)}
          </Form.Item>
          <Form.Item label="商品条码">
            ...
            })(<Input placeholder="请输入商品条码" className={this.readOnly ? styles['readOnly']: ''} readOnly={this.readOnly}/>)}
          </Form.Item>
          <Form.Item label="供货商">
            {this.readOnly ? this.props.form.getFieldValue('storeId')
          ...
          <Form.Item label="供应商商品ID">
            {getFieldDecorator('storeProductId')(<Input placeholder="请填写供货商商品ID" className={this.readOnly ? styles['readOnly']: ''}/>)}
            {!id && <Button readOnly={this.readOnly} onClick={()=>this.sync1688Sku()} >同步1688规格信息</Button>}
          </Form.Item>
          {
            interceptionVisible ?
          ...
          <Form.Item label="商品视频封面">
            {getFieldDecorator('videoCoverUrl')(<UploadView readOnly={this.readOnly} placeholder="上传视频封面" listType="picture-card" listNum={1} size={0.3} />)}
          </Form.Item>
          <Form.Item label="商品视频">
            {getFieldDecorator('videoUrl')(<UploadView readOnly={this.readOnly} placeholder="上传视频" fileType='video' listType="picture-card" listNum={1} size={5} />)}
          </Form.Item>
          <Form.Item label="商品主图" required={true}>
            ...
            })(<UploadView readOnly={this.readOnly} placeholder="上传主图" listType="picture-card" listNum={1} size={.3} />)}
          </Form.Item>
          <Form.Item
            label="商品图片"
            required={true}
        ...
        <Card title="物流信息" style={{ marginTop: 10 }}>
          <Form.Item label="物流体积">
            {getFieldDecorator('bulk')(<Input placeholder="请设置物流体积" className={this.readOnly ? styles['readOnly']: ''} readOnly={this.readOnly}/>)}
          </Form.Item>
          <Form.Item label="物流重量">
            {getFieldDecorator('weight')(<Input placeholder="请设置物流重量" className={this.readOnly ? styles['readOnly']: ''} readOnly={this.readOnly}/>)}
          </Form.Item>
