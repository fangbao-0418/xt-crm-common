...
@connect(state => ({
    level1List: state['goods.category'].level1List,
    level2List: state['goods.category'].level2List,
    level3List: state['goods.category'].level3List
}))
@Form.create()
export default class extends Component {
    ...
    handleSearch = (params) => {
        ...
        dispatch['goods.category'].getList(params);
    }
    ...
    onConfirm = (item) => {
        const { dispatch } = this.props;
        dispatch['goods.category'].delCategory(item, () => {
            const { level, id } = item;
    ...
    onOk = () => {
        ...
        validateFields((err, values) => {
            ...
            if (!err) {
                ...
                dispatch['goods.category'].addCategory(payload);
                this.setState({
                    visible: false
                })
            }
    ...
    onEdit = () => {
        ...
        validateFields((err, values) => {
            if (!err) {
                ...
                dispatch['goods.category'].editCategory(payload, { level: currentItem.level, parentId: currentItem.parentId });
                this.setState({
                    editVisible: false
                })
            }
