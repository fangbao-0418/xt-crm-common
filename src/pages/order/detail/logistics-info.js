    ...
    Array.isArray(logistics.orderPackageList) && logistics.orderPackageList.length > 0 ?
      <div>
        ...
        {map(logistics.orderPackageList, (data, index) => (
          <Row key={index} gutter={24} key={index} style={{ marginBottom: 8 }} >
            ...
            <Col span={6} style={{ textAlign: 'right' }}>
              ...
              <Button type='primary' onClick={() => showModal(data)}>修改物流信息</Button>
            </Col>
          </Row>
        ))}
        <DeliveryModal visible={visible} mainorderInfo={mainorderInfo}
          logistics={orderInfo}
          title="修改物流信息"
          onSuccess={onOk}
          data={data}
