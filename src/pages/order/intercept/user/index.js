...
import { Input, Select, Button, Row, Col, Form, Table, Card, Divider, message, Modal, Checkbox, Spin } from 'antd';
...
export default class extends Component {
    ...
    render() {
        ...
        const blockListByHeadCount = difference(concat((blockListByHead.checkedMemberIds || []), idsByLevelByHead.addIds || []), idsByLevelByHead.delIds || []).length;
        const blockListByDistrictCount = difference(concat((blockListByDistrict.checkedMemberIds || []), idsByLevelByDistrict.addIds || []), idsByLevelByDistrict.delIds || []).length;
        const blockListByPartnerCount = difference(concat((blockListByPartner.checkedMemberIds || []), idsByLevelByPartner.addIds || []), idsByLevelByPartner.delIds || []).length;
        return (
            <div>
                <Card>
                    {this.renderForm()}
                ...
                >
                    <div style={{ paddingBottom: '20px' }}>
                        <Checkbox
                            ...
                            onClick={() => { this.changePrivileged(blockListByHead) }}
                        >
                            全部团长
                        </Checkbox>
                    ...
                    <div style={{ paddingBottom: '20px' }}>
                        <Checkbox
                            ...
                            onClick={() => { this.changePrivileged(blockListByDistrict) }}
                        >
                            全部社区管理员
                        </Checkbox>
                    ...
                    <div style={{ paddingBottom: '20px' }}>
                        <Checkbox checked={blockListByPartner && blockListByPartner.privileged} onClick={() => { this.changePrivileged(blockListByPartner) }}>全部城市合伙人</Checkbox>
                        ...
                        <FormItem label="等级">
                            {
                                getFieldDecorator('memberType', {
                                    ...
                                    <Select placeholder="请选择用户等级">
                                        {
                                            levelList.map(item => <Option value={item.value} key={item.value}>{item.key}</Option>)
                                        }
                                    </Select>
                                )
                            }
                        ...
                        <FormItem label="拦截权限">
                            {
                                getFieldDecorator('orderInterception', {
                                    ...
                                    <Select placeholder="请选择用户拦截权限">
                                        {
                                            permissionList.map(item => <Option value={item.value} key={item.value}>{item.key}</Option>)
                                        }
                                    </Select>
                                )
                            }
