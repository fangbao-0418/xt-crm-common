...
@connect(state => ({
  modal: state['shop.hd.popcommission'].configModal,
  currentCategory: state['shop.hd.popcommission'].currentCategory
}))
@Form.create()
export default class ConfigModal extends Component {
  ...
  handleOk = () => {
    ...
    validateFields((err, { commissionType, agencyRate, companyRate, isCoverChildCategory }) => {
      ...
      dispatch['shop.hd.popcommission'].updateCategory(params);
    });
  }
  ...
  handleCancel = () => {
    ...
    dispatch({
      type: 'shop.hd.popcommission/saveDefault',
      payload: {
        configModal: {
          visible: false
        }
