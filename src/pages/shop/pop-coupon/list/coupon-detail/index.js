...
import { invalidCoupon, getCouponDetail, getCouponTasks } from '@/pages/shop/pop-coupon/api'
    ...
    <Card>
      <Tabs onChange={onChange} activeKey={activeKey}>
        ...
        <TabPane tab='批量发送记录' key='2'>
          <SendRecord code={query.code} />
          {/* <Table
            loading={loading}
            rowKey='id'
            columns={releaseRecordsColumns}
            dataSource={couponTasks.map(v => ({ ...v, couponId: match.params.id }))}
          >
          </Table> */}
        </TabPane>
      </Tabs>
