...
import { getCouponDetail, getCategoryList, saveCouponInfo, importShop } from '@/pages/shop/pop-coupon/api'
...
class CouponInfo extends React.Component {
  ...
  handleSave = () => {
    formRef.validateFields(async (err, vals) => {
      ...
      if(vals.useTimeType === 2){
        if(startDays===''||endDays===''){
          return void message.error('请输入领券当日起多少天至多少天内可用')
        }
        if(parseInt(startDays)>=parseInt(endDays)){
          return void message.error('领券天数后一个数字需大于前一个数字')
        }
      ...
      >
        ...
        >
          ...
          <FormItem
            ...
            required
            inner={(form) => {
              const placeholder = ['请选择开始时间', '请选择结束时间']
              const value=form.getFieldValue('receiveTime') instanceof Array ? form.getFieldValue('receiveTime'):[]
              return (
                 <>
                 ...
                 >
                   <DatePicker
                     ...
                     onChange={(date, dateString) => {
                       value[0] = date
                       //如果点击过长期按钮并且开始结束时间存在，则结束时间=开始时间+50年
                       if (isLong&&value[1]&&value[0]) {
                         const day=new Date(value[0])
                         day.setFullYear(day.getFullYear()+50)
                         value[1] = moment(day)
                       }
                       form.setFieldsValue({
                         'receiveTime': value[0] || value[1] ? value : undefined
                       })
                     }}
                   />
              ...
              onChange: (e) => {
                ...
                if ([1,2].includes(value)) {
                  ...
                  if (useTimeRange[0] && useTimeRange[1]) {
                    const msg = useTimeRange[0] >= useTimeRange[1] ? '使用开始时间必须小于结束时间' : (useTimeRange[1] < moment()?'使用结束时间必须大于当前时间' : '')
                    this.setState({
                      useTimeErrorMsg: msg
                    })
                  }
                }
                //更新useTimeType的值，刷新领取时间的校验
                form.setValues({
                  useTimeType: value
                })
                form.props.form.validateFields(['receiveTime'], { force: true })
                  ...
                  <DatePicker.RangePicker
                    ...
                    onChange={date => {
                      if (date[0] && date[1]) {
                        ...
                        const msg = date[0] >= date[1] ? '使用开始时间必须小于结束时间' : (date[1] < moment()?'使用结束时间必须大于当前时间' : '')
                        this.setState({
                          useTimeErrorMsg: useTimeType === 0 && msg
                        })
                      } else {
                        this.setState({
                          useTimeErrorMsg: ''
              ...
              label: (
                <>
                  ...
                  <InputNumber
                    ...
                    precision={0}
                    value={startDays}
                    onChange={startDays => this.setState({ startDays })}
                  />
                  <span className='ml10'>至</span>
                   <InputNumber
                    className='ml10 short-input'
                    min={0}
                    precision={0}
                    value={endDays}
                    onChange={endDays => this.setState({ endDays })}
                  />
                  <span className='ml10'>天之间可用</span>
                </>
              ),
              value: 2
            }]}
