  ...
  render () {
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'type',
        width: 100,
        render: (text: any) => <>{uliveActivityType.getValue(text)}</>
      },
      {
        title: '活动状态',
        dataIndex: 'status',
              ...
              <FormItem label='活动类型'>
                {getFieldDecorator('type')(
                  <Select placeholder='请选择活动类型' style={{ width: 150 }}>
                    ...
                    {uliveActivityType
                      .getArray()
                      .filter(val => (this.props.uliveActivityType || this.props.types || [1, 2, 3]).includes(val.key))
                      .map((val, i) => (
