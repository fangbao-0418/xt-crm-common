...
import { Switch, Card, Divider, Tabs, message, Modal, Input, Select, Form, Spin } from 'antd';
import styles from './index.module.sass';
import Stock from './stock';
import Order from '@/pages/order/order-table';
import Refund from '@/pages/order/refund/list';
import { getPrivilegeById, openPrivilege, closePrivilege, getAddressByMemberId, getBaseAddress, updateAddressByMemberId, addAddressByMemberId } from './api';
...
const CascaderByArea = ({ value, onChange, baseAddress }) => {
    ...
    return <div>
        ...
        <Select value={currentDistrict.id} placeholder="选择区" style={{ width: 120, marginLeft: 8 }} onChange={changeDistrict}>
...
export default class extends Component {
    ...
    render() {
        const {
            isPrivilege, address, addressVisible, addressLoading, baseAddress,
            currentProvince, currentCity, currentDistrict, name, phone, detailAddress
        } = this.state;
        ...
        return (
            <Card>
                <div className={styles['setting']}>
                    <span>拦截发货权限<Switch style={{ marginLeft: 8 }} checked={isPrivilege} onChange={this.privilegeChange} /></span>
                    <span className={styles['address']}>
                        <span>售后地址：</span>
                        {
                            isNil(address) ?
                                <a onClick={this.changeAddress}>设置</a> :
                                <span>
                                    <span>{`${address.province} ${address.city} ${address.district} ${address.street}`}</span>
                                    <a onClick={this.changeAddress}>修改</a>
                                </span>
                        }
                    </span>
                </div>
    ...
    onOk = () => {
        ...
        const { address, currentDistrict, currentCity, currentProvince } = this.state;
