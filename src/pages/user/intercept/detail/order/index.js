...
export default class extends Component {
  componentDidMount() {
    const { dispatch, orderInfo: { current, size } } = this.props;
  ...
  render() {
    const { orderInfo: { records } } = this.props;
    const columns = [
      {
        title: '订单编号',
        dataIndex: 'orderCode'
      ...
      <Form {...formItemLayout} className={styles['search-form']}>
        <Row gutter={24}>
          ...
          <Col span={4}>
            <FormItem label='下单人电话'>
              {
                getFieldDecorator('createPhoneNumber')(
                  <Input />
                )
              }
            </FormItem>
            ...
            <FormItem label="订单状态">
              {
                getFieldDecorator('orderState', {
                  ...
                  <Select>
                    ...
                    <Option value={1} key={1}>待拦截</Option>
                    <Option value={2} key={2}>待发货</Option>
                    <Option value={3} key={3}>已发货</Option>
                    <Option value={4} key={4}>已完成</Option>
                    <Option value={5} key={5}>关闭</Option>
                  </Select>
                )
              }
            </FormItem>
        ...
        <Row>
          <Col span={24} style={{ textAlign: 'right' }}>
            ...
            <Button onClick={this.onShowModal} style={{ marginLeft: 8 }}>清除条件</Button>
          </Col>
        </Row>
      </Form>
    )
